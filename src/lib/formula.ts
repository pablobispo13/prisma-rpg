 ...
 *
 * Suporta: números, + - * /, parênteses e os 5 atributos como variáveis,
 * em português ou inglês, sem acento e case-insensitive:
 ...
 *
 * Ex: "3 + agilidade + vigor" · "25 + forca*2 + vigor*3" · "10 + (agi + vig) / 2"
 */
...
const VARIABLE_ALIASES: Record<string, keyof AttributeValues> = {
...
type Token =
  ...
  | { kind: "variable"; name: keyof AttributeValues }
  | { kind: "op"; value: "+" | "-" | "*" | "/" }
  | { kind: "lparen" }
  | { kind: "rparen" };
...
function tokenize(formula: string): Token[] {
  ...
  while (i < src.length) {
    ...
    if (/[a-z]/.test(ch)) {
      ...
      const attr = VARIABLE_ALIASES[word];
      if (!attr) throw new Error(`Variável desconhecida: "${word}"`);
...
function parse(tokens: Token[], attrs: AttributeValues): number {
  ...
  function parseFactor(): number {
    ...
    if (t.kind === "variable") { pos++; return attrs[t.name]; }
    if (t.kind === "lparen") {
      pos++;
...
export function validateFormula(formula: string): string | null {
  try {
    evaluateFormula(formula, {
      strength: 1, agility: 1, vigor: 1, intellect: 1, presence: 1,
    });
    return null;
  } catch (err) {
